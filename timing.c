...
SP_PRIV void
time_set_ms(struct time* time, unsigned int ms) {
#ifdef _WIN32
...
SP_PRIV void
time_add(const struct time* a, const struct time* b, struct time* result) {
#ifdef _WIN32
...
SP_PRIV void
time_sub(const struct time* a, const struct time* b, struct time* result) {
#ifdef _WIN32
  result->ticks = a->ticks - b->ticks;
#else
...
SP_PRIV bool
time_greater(const struct time* a, const struct time* b) {
#ifdef _WIN32
...
SP_PRIV void
time_as_timeval(const struct time* time, struct timeval* tv) {
#ifdef _WIN32
  LARGE_INTEGER frequency;
  QueryPerformanceFrequency(&frequency);
...
SP_PRIV unsigned int
time_as_ms(const struct time* time) {
#ifdef _WIN32
  LARGE_INTEGER frequency;
  QueryPerformanceFrequency(&frequency);
...
SP_PRIV void
timeout_start(struct timeout* timeout, unsigned int timeout_ms) {
...
SP_PRIV void
timeout_limit(struct timeout* timeout, unsigned int limit_ms) {
...
SP_PRIV bool
timeout_check(struct timeout* timeout) {
...
SP_PRIV void
timeout_update(struct timeout* timeout) {
  timeout->calls_started = true;
}

#ifndef _WIN32
SP_PRIV struct timeval*
timeout_timeval(struct timeout* timeout) {
  if(timeout->ms == 0)
    return NULL;
...
SP_PRIV unsigned int
timeout_remaining_ms(struct timeout* timeout) {
