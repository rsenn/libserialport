...
static char*
wc_to_utf8(PWCHAR wc_buffer, ULONG wc_bytes) {
...
static char*
get_root_hub_name(HANDLE host_controller) {
  ...
  if(!DeviceIoControl(host_controller, IOCTL_USB_GET_ROOT_HUB_NAME, 0, 0, &root_hub_name, sizeof(root_hub_name), &size, NULL))
    return NULL;
...
static char*
get_external_hub_name(HANDLE hub, ULONG connection_index) {
  ...
  if(!DeviceIoControl(hub, IOCTL_USB_GET_NODE_CONNECTION_NAME, &ext_hub_name, sizeof(ext_hub_name), &ext_hub_name, sizeof(ext_hub_name), &size, NULL))
    return NULL;
  ...
  if(!DeviceIoControl(hub, IOCTL_USB_GET_NODE_CONNECTION_NAME, ext_hub_name_wc, size, ext_hub_name_wc, size, &size, NULL)) {
...
static char*
get_string_descriptor(HANDLE hub_device, ULONG connection_index, UCHAR descriptor_index) {
  ...
  if(!DeviceIoControl(hub_device, IOCTL_USB_GET_DESCRIPTOR_FROM_NODE_CONNECTION, desc_req, size, desc_req, size, &size, NULL) || size < 2 ||
     desc->bDescriptorType != USB_STRING_DESCRIPTOR_TYPE || desc->bLength != size - sizeof(*desc_req) || desc->bLength % 2)
    return NULL;
...
static void
enumerate_hub_ports(struct sp_port* port, HANDLE hub_device, ULONG nb_ports, const char* parent_path, DEVINST dev_inst) {
  ...
  for(index = 1; index <= nb_ports; index++) {
    ...
    if(!DeviceIoControl(hub_device, IOCTL_USB_GET_NODE_CONNECTION_INFORMATION_EX, connection_info_ex, size, connection_info_ex, size, &size, NULL)) {
      /*
      ...
      if(!DeviceIoControl(hub_device, IOCTL_USB_GET_NODE_CONNECTION_INFORMATION, connection_info, size, connection_info, size, &size, NULL)) {
    ...
    if(connection_info_ex->DeviceIsHub) {
      ...
      if(connection_info_ex->DeviceDescriptor.iManufacturer)
        port->usb_manufacturer = get_string_descriptor(hub_device, index, connection_info_ex->DeviceDescriptor.iManufacturer);
      if(connection_info_ex->DeviceDescriptor.iProduct)
        port->usb_product = get_string_descriptor(hub_device, index, connection_info_ex->DeviceDescriptor.iProduct);
      if(connection_info_ex->DeviceDescriptor.iSerialNumber) {
        port->usb_serial = get_string_descriptor(hub_device, index, connection_info_ex->DeviceDescriptor.iSerialNumber);
...
static void
enumerate_hub(struct sp_port* port, const char* hub_name, const char* parent_path, DEVINST dev_inst) {
  ...
  if(DeviceIoControl(hub_device, IOCTL_USB_GET_NODE_INFORMATION, &hub_info, size, &hub_info, size, &size, NULL))
    ...
    enumerate_hub_ports(port, hub_device, hub_info.u.HubInformation.HubDescriptor.bNumberOfPorts, parent_path, dev_inst);

  CloseHandle(hub_device);
}

static void
enumerate_host_controller(struct sp_port* port, HANDLE host_controller_device, DEVINST dev_inst) {
  char* root_hub_name;
...
static void
get_usb_details(struct sp_port* port, DEVINST dev_inst_match) {
  HDEVINFO device_info;
  SP_DEVINFO_DATA device_info_data;
  ULONG i, size = 0;

  device_info = SetupDiGetClassDevs(&GUID_CLASS_USB_HOST_CONTROLLER, NULL, NULL, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);
  ...
  for(i = 0; SetupDiEnumDeviceInfo(device_info, i, &device_info_data); i++) {
    ...
    if(!SetupDiGetDeviceInterfaceDetail(device_info, &device_interface_data, NULL, 0, &size, NULL) && GetLastError() != ERROR_INSUFFICIENT_BUFFER)
      continue;

    if(!(device_detail_data = malloc(size)))
      continue;
    device_detail_data->cbSize = sizeof(*device_detail_data);
    if(!SetupDiGetDeviceInterfaceDetail(device_info, &device_interface_data, device_detail_data, size, &size, NULL)) {
    ...
    host_controller_device = CreateFile(device_detail_data->DevicePath, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
...
SP_PRIV enum sp_return
get_port_details(struct sp_port* port) {
  /*
  ...
  for(i = 0; SetupDiEnumDeviceInfo(device_info, i, &device_info_data); i++) {
    ...
    device_key = SetupDiOpenDevRegKey(device_info, &device_info_data, DICS_FLAG_GLOBAL, 0, DIREG_DEV, KEY_QUERY_VALUE);
    ...
    if(port->transport == SP_TRANSPORT_USB) {
      ...
      do {
        ...
        if(CM_Get_Device_IDA(dev_inst, device_id, sizeof(device_id), 0) != CR_SUCCESS || strncmp(device_id, "USB\\", 4))
          continue;

        /* Discard one layer for composite devices. */
        char compat_ids[512], *p = compat_ids;
        size = sizeof(compat_ids);
        if(CM_Get_DevNode_Registry_PropertyA(dev_inst, CM_DRP_COMPATIBLEIDS, 0, &compat_ids, &size, 0) == CR_SUCCESS) {
          while(*p) {
      ...
      handle = CreateFileA(escaped_port_name, GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, 0);
...
SP_PRIV enum sp_return
list_ports(struct sp_port*** list) {
  HKEY key;
  TCHAR *value, *data;
  ...
  if((result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("HARDWARE\\DEVICEMAP\\SERIALCOMM"), 0, KEY_QUERY_VALUE, &key)) != ERROR_SUCCESS) {
  ...
  if((result = RegQueryInfoKey(key, NULL, NULL, NULL, NULL, NULL, NULL, NULL, &max_value_len, &max_data_size, NULL, NULL)) != ERROR_SUCCESS) {
    SetLastError(result);
