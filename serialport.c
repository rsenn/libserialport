...
SP_API enum sp_return
sp_get_port_by_name(const char* portname, struct sp_port** port_ptr) {
  struct sp_port* port;
...
SP_PRIV struct sp_port**
list_append(struct sp_port** list, const char* portname) {
  void* tmp;
  size_t count;
...
SP_API void
sp_free_port_list(struct sp_port** list) {
  unsigned int i;
...
static enum sp_return
restart_wait(struct sp_port* port) {
  DWORD wait_result;
...
#ifdef _WIN32
static enum sp_return
await_write_completion(struct sp_port* port) {
  TRACE("%p", port);
  DWORD bytes_written;
  BOOL result;
...
static enum sp_return
restart_wait_if_needed(struct sp_port* port, unsigned int bytes_read) {
  DWORD errors;
  COMSTAT comstat;
...
sp_blocking_read(struct sp_port* port, void* buf, size_t count, unsigned int timeout_ms) {
  ...
  /* Set timeout. */
  if(port->timeouts.ReadIntervalTimeout != 0 || port->timeouts.ReadTotalTimeoutMultiplier != 0 || port->timeouts.ReadTotalTimeoutConstant != timeout_ms) {
    port->timeouts.ReadIntervalTimeout = 0;
    port->timeouts.ReadTotalTimeoutMultiplier = 0;
...
sp_nonblocking_read(struct sp_port* port, void* buf, size_t count) {
  ...
  /* Set timeout. */
  if(port->timeouts.ReadIntervalTimeout != MAXDWORD || port->timeouts.ReadTotalTimeoutMultiplier != 0 || port->timeouts.ReadTotalTimeoutConstant != 0) {
    port->timeouts.ReadIntervalTimeout = MAXDWORD;
    port->timeouts.ReadTotalTimeoutMultiplier = 0;
    port->timeouts.ReadTotalTimeoutConstant = 0;
    if(SetCommTimeouts(port->hdl, &port->timeouts) == 0)
...
static enum sp_return
add_handle(struct sp_event_set* event_set, event_handle handle, enum sp_event mask) {
  void* new_handles;
...
SP_API void
sp_free_event_set(struct sp_event_set* event_set) {
  TRACE("%p", event_set);
...
#ifdef _WIN32
  if(WaitForMultipleObjects(event_set->count, event_set->handles, FALSE, timeout_ms ? timeout_ms : INFINITE) == WAIT_FAILED)
...
static enum sp_return
get_baudrate(int fd, int* baudrate) {
  void* data;
...
static enum sp_return
set_baudrate(int fd, int baudrate) {
  void* data;
...
#ifdef USE_TERMIOX
static enum sp_return
get_flow(int fd, struct port_data* data) {
  void* termx;
...
static enum sp_return
set_flow(int fd, struct port_data* data) {
  void* termx;
...
static enum sp_return
get_config(struct sp_port* port, struct port_data* data, struct sp_port_config* config) {
  unsigned int i;
...
static enum sp_return
set_config(struct sp_port* port, struct port_data* data, const struct sp_port_config* config) {
  unsigned int i;
#ifdef __APPLE__
...
SP_API enum sp_return
sp_get_signals(struct sp_port* port, enum sp_signal* signals) {
